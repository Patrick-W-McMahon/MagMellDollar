...
#if defined(NDEBUG)
# error "Magmelldollar cannot be compiled without assertions."
#endif
...
const std::string strMessageMagic = "Magmelldollar Signed Message:\n";
...
static void UpdateMempoolForReorg(DisconnectedBlockTransactions &disconnectpool, bool fAddToMempool)
{
    ...
    while (it != disconnectpool.queuedTx.get<insertion_order>().rend()) {
        ...
        if (!fAddToMempool || (*it)->IsCoinBase() || (*it)->IsTreasuryMint() ||
...
static bool CheckInputsFromMempoolAndCache(const CTransaction& tx, CValidationState& state, const CCoinsViewCache& view, const CTxMemPool& pool,
    ...
    assert(!tx.IsCoinBase() && !tx.IsTreasuryMint());
                              ...
                              bool bypass_limits, const CAmount& nAbsurdFee, std::vector<COutPoint>& coins_to_uncache, bool test_accept)
{
    ...
    if (tx.IsCoinBase())//but Treasury Mint can be at any position in the transactions.
...

    {
        ...
        for (const CTxIn &txin : tx.vin) {
            const Coin &coin = view.AccessCoin(txin.prevout);
            if (coin.IsCoinBase()) {//Treasury Mint can be spent right away.
        ...
        for (CTxMemPool::txiter it : allConflicting)
        {
            LogPrint(BCLog::MEMPOOL, "replacing tx %s with %s for %s MMD additional fees, %d delta bytes\n",
...
CAmount GetBlockSubsidy(int nHeight, const Consensus::Params& consensusParams)
{
    ...
    CAmount nSubsidy = consensusParams.blockBaseSubsidy * COIN;
...
{
    // mark inputs spent
    if (!tx.IsCoinBase() && !tx.IsTreasuryMint()) {
...
DisconnectResult CChainState::DisconnectBlock(const CBlock& block, const CBlockIndex* pindex, CCoinsViewCache& view)
{
    ...
    for (int i = block.vtx.size() - 1; i >= 0; i--) {
        ...
        bool is_treasuryMint = tx.IsTreasuryMint();
        ...
        for (size_t o = 0; o < tx.vout.size(); o++) {
            if (!tx.vout[o].scriptPubKey.IsUnspendable()) {
                ...
                if (!is_spent || tx.vout[o] != coin.out || pindex->nHeight != coin.nHeight || (is_coinbase != coin.fCoinBase || is_treasuryMint != coin.fCoinBase)) {
...
void ThreadScriptCheck() {
    RenameThread("magmelldollar-scriptch");
                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck)
{
    ...
    for (unsigned int i = 0; i < block.vtx.size(); i++)
    {
        ...
        if (!tx.IsCoinBase() && !tx.IsTreasuryMint())
        {
            ...
            if (txfee - chainparams.GetConsensus().burnFee < 0) {
                return error("%s: Transation (%s): transaction fee of %s is not enough to cover burn fee of %s", __func__, 
                    tx.GetHash().ToString(), txfee, chainparams.GetConsensus().burnFee);
            }
            nFees += (txfee - chainparams.GetConsensus().burnFee);
            if (!MoneyRange(nFees)) {
...
bool CheckBlock(const CBlock& block, CValidationState& state, const Consensus::Params& consensusParams, bool fCheckPOW, bool fCheckMerkleRoot)
{
    ...
    for (unsigned int i = 1; i < block.vtx.size(); i++)
        if (block.vtx[i]->IsCoinBase() && !block.vtx[i]->IsTreasuryMint())
