...
UniValue getinfo(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw runtime_error(
            ...
            "  \"balance\": xxxxxxx,         (numeric) the total magmelldollar balance of the wallet\n"
    ...
    UniValue operator()(const CScriptID &scriptID) const {
        ...
        if (pwalletMain && pwalletMain->GetCScript(scriptID, subscript)) {
            ...
            BOOST_FOREACH(const CTxDestination& addr, addresses)
                a.push_back(CMagmelldollarAddress(addr).ToString());
...
UniValue validateaddress(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            "validateaddress \"magmelldollaraddress\"\n"
            "\nReturn information about the given magmelldollar address.\n"
            "\nArguments:\n"
            "1. \"magmelldollaraddress\"     (string, required) The magmelldollar address to validate\n"
            "\nResult:\n"
            "{\n"
            "  \"isvalid\" : true|false,       (boolean) If the address is valid or not. If not, this is the only property returned.\n"
            "  \"address\" : \"magmelldollaraddress\", (string) The magmelldollar address validated\n"
...
#ifdef ENABLE_WALLET
        // Case 1: Magmelldollar address and we have full public key:
        CMagmelldollarAddress address(ks);
...
UniValue createmultisig(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 2)
    {
        string msg = "createmultisig nrequired [\"key\",...]\n"
            ...
            "2. \"keys\"       (string, required) A json array of keys which are magmelldollar addresses or hex-encoded public keys\n"
            "     [\n"
            "       \"key\"    (string) magmelldollar address or hex-encoded public key\n"
            "       ,...\n"
            "     ]\n"
    ...
    CMagmelldollarAddress address(innerID);
...
UniValue verifymessage(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 3)
        throw runtime_error(
            "verifymessage \"magmelldollaraddress\" \"signature\" \"message\"\n"
            "\nVerify a signed message\n"
            "\nArguments:\n"
            "1. \"magmelldollaraddress\"  (string, required) The magmelldollar address to use for the signature.\n"
...
UniValue signmessagewithprivkey(const UniValue& params, bool fHelp)
{
    ...
    CMagmelldollarSecret vchSecret;
...
{
    if (type == 2) {
        address = CMagmelldollarAddress(CScriptID(hash)).ToString();
    } else if (type == 1) {
        address = CMagmelldollarAddress(CKeyID(hash)).ToString();
    } else {
        return false;
    }
    return true;
...
bool getAddressesFromParams(const UniValue& params, std::vector<std::pair<uint160, int> > &addresses)
{
    if (params[0].isStr()) {
        ...
        for (std::vector<UniValue>::iterator it = values.begin(); it != values.end(); ++it) {
            ...
            CMagmelldollarAddress address(it->get_str());
            uint160 hashBytes;
            int type = 0;
            if (!address.GetIndexKey(hashBytes, type)) {
                throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid address");
