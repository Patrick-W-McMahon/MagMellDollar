    ...
    if(role == Qt::DisplayRole || role == Qt::EditRole)
    {
        ...
        case Amount:
            if (rec->recipient.amount == 0 && role == Qt::DisplayRole)
                ...
                return MagmelldollarUnits::format(walletModel->getOptionsModel()->getDisplayUnit(), rec->recipient.amount, false, MagmelldollarUnits::separatorNever);
            else
                return MagmelldollarUnits::format(walletModel->getOptionsModel()->getDisplayUnit(), rec->recipient.amount);
        }
    }
...
{
    return (this->walletModel->getOptionsModel() != NULL) ? tr("Requested") + " ("+MagmelldollarUnits::name(this->walletModel->getOptionsModel()->getDisplayUnit()) + ")" : "";
}
