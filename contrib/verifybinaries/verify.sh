...
###   This script attempts to download the signature file SHA256SUMS.asc from magmelldollar.org
...
WORKINGDIR="/tmp/magmelldollar"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test/"
BASEDIR="https://magmelldollar.org/bin/"
VERSIONPREFIX="magmelldollar-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'magmelldollar-',
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the Magmelldollar Core binary release signing key installed?"
   fi
