...
The RPM spec file provided here is for Magmelldollar-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://magmelldollar.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[Magmelldollar git repository](https://github.com/magmelldollar/magmelldollar) but are not part of
...
Currently only `Source10` is used, for the example `magmelldollar.conf` file.
...
in the magmelldollar git repository. At present only an SVG version of the Magmelldollar
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the magmelldollar
...
needed when 0.12.1 is released, a proper fix is already in the Magmelldollar git
master branch.
...
idea to build Magmelldollar Core against a static version of that library compiled
...
The `magmelldollar.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `magmelldollar-server` package. At this point, most RPM based
...
The `magmelldollar-server` package should have SELinux support. How to properly do
...
Magmelldollar-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/magmelldollar/). He has been
packaging Magmelldollar for Fedora at least since 2012.
...
4. I renamed the `magmelldollar` package that contains the Qt GUI to `magmelldollar-core` as
that appears to be how the general population refers to it, in contrast to
`magmelldollar-xt` or `magmelldollar-classic`. I wanted to make sure the general population
