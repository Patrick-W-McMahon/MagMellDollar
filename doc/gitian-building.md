...
*Setup instructions for a Gitian build of Magmelldollar Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Magmelldollar
...
- [Building Magmelldollar Core](#building-magmelldollar-core)
...
- [LXC](https://linuxcontainers.org/), see also [Gitian host docker container](https://github.com/gdm85/tenku/tree/master/docker/gitian-magmelldollar-host/README.md).
...
Clone the git repositories for magmelldollar and Gitian.
...
in the magmelldollar repository under 'Fetch and create inputs' to install sources which require
manual intervention. Also optionally follow the next step: 'Seed the Gitian sources cache
and offline git repositories' which will fetch the remaining files required for building
offline.

Building Magmelldollar Core
----------------

To build Magmelldollar Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the magmelldollar repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/magmelldollar/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/magmelldollar-project/magmelldollar
...
For example:
```bash
URL=https://github.com/thrasher-/magmelldollar.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit magmelldollar=${COMMIT} --url magmelldollar=${URL} ../magmelldollar/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit magmelldollar=${COMMIT} --url magmelldollar=${URL} ../magmelldollar/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit magmelldollar=${COMMIT} --url magmelldollar=${URL} ../magmelldollar/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the magmelldollar git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../magmelldollar/contrib/gitian-descriptors/*|sort|uniq )
...
MMDPATH=/some/root/path/magmelldollar
SIGPATH=/some/root/path/magmelldollar-detached-sigs

./bin/gbuild --url magmelldollar=${MMDPATH},signature=${SIGPATH} ../magmelldollar/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/magmelldollar-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/magmelldollar-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/magmelldollar-osx-build.assert
```
...
[gitian.sigs.ltc](https://github.com/magmelldollar-project/gitian.sigs.ltc/) repository, or if that's not possible create a pull
